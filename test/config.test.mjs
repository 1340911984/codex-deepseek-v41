...
test("catalog adds one DeepSeek V4.1 Flash entry with honest reasoning levels", () => {
  const catalog = buildCatalog({ models: [TEMPLATE] });
  const model = catalog.models[0];
  assert.equal(model.slug, "deepseek/deepseek-flash");
  assert.equal(model.display_name, "DeepSeek V4.1 Flash");
  assert.equal(model.default_reasoning_level, "high");
  assert.deepEqual(model.supported_reasoning_levels.map(({ effort }) => effort), ["low", "high", "max"]);
  assert.equal(model.base_instructions, "You are Codex, powered by DeepSeek V4.1 Flash.");
  assert.deepEqual(model.input_modalities, ["text", "image"]);
  assert.equal(model.prefer_websockets, false);
  assert.equal(model.auto_compact_token_limit, 700_000);
});
