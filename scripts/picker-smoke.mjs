...
child.stdout.on("data", (chunk) => {
  ...
  while (buffer.includes("\n")) {
    ...
    if (message.id === 2) {
      ...
      const model = message.result?.data?.find((item) => item.model === "deepseek/deepseek-flash");
      assert.ok(model, "DeepSeek V4.1 Flash is absent from Codex model/list");
      assert.equal(model.displayName, "DeepSeek V4.1 Flash");
